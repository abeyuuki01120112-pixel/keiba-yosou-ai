...
作業中説明・進捗報告・CHECKPOINT完了報告・問題点・判定・次に決める項目など、
ユーザー（ChatGPTとの橋渡し役）向けの説明文は、すべて日本語で出力すること
（CHECKPOINT13.5Bで明示指示）。コード・関数名・型名・ファイル名・CLIコマンド・
enum等はこの限りではなく、英語のままでよい。
